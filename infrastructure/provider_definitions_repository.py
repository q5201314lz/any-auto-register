...
_BUILTIN_DEFINITIONS: list[dict] = [
    ...
    },
    {
        ...
        "label": "本地邮箱池",
        "description": "导入本地账号池：支持邮箱收码账号、iCloud 接码地址，以及已注册账号的密码 + MFA 登录",
        ...
        "fields": [
            {
                ...
                "category": "connection",
                "hint": "可选；每行一条账号。接码地址支持邮箱后用3个及以上连字符分隔；另支持心蓝格式、邮箱----密码----MFA、邮箱|密码|2FA。",
            },
            {
                ...
                "type": "textarea",
                "category": "auth",
                "hint": "可选；直接粘贴账号池。接码地址支持 邮箱 + 3个及以上连字符 + URL；已注册账号支持 邮箱----密码----MFA 或 邮箱|密码|2FA。",
            },
            {
    ...
    # ── sms ──────────────────────────────────────────────────────────
    {
        ...
        "fields": [
            ...
            {"key": "register_phone_max_attempts", "label": "手机号验证尝试次数", "placeholder": "5", "hint": "手机号被占用、短信超时或验证码失败时自动换号重试，最多 5 次；账号级请求过频交给任务调度器稍后重试。"},
            {"key": "register_account_max_attempts", "label": "注册失败补跑倍数", "placeholder": "1", "hint": "1 表示每个目标只执行一遍；大于 1 才会在失败后补跑。"},
